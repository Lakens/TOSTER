...
abstract: |
  ...
  equivalence testings still appears to be low.
  ...
  In recent years, I have made significant changes to the TOSTER package
...
The `t_TOST` function, and related `*_TOST` functions, are designed to be comprehensive — they run three tests (one two-tailed and two one-sided), compute standardized effect sizes, and produce verbose output. For researchers who want the full picture, this is ideal. However, for users who *only* want to test a single hypothesis of equivalence (or minimal effects), the output can feel unnecessarily heavy. The three-test structure may be confusing when the researcher only cares about one of those tests.

To address this, I created the `simple_htest` function as a lighter interface. It returns a standard R `htest` object — the same class returned by `t.test()` and `wilcox.test()` — but with expanded `alternative` options that include `"equivalence"` and `"minimal.effect"`. This means researchers can perform equivalence testing with the same familiar interface they already use for standard hypothesis tests.

Alongside `simple_htest`, I also developed a set of helper functions for working with any `htest` object. These helpers make it easy to convert results to data frames (`df_htest`), generate (almost) publication-ready text descriptions (`describe_htest`), create plots (`plot_htest_est`), and bridge between the verbose TOST output and the simpler `htest` ecosystem (`as_htest`). Importantly, these tools work not only with TOSTER functions but with any base R hypothesis test.

## simple_htest

The `simple_htest` function supports `t.test` and `wilcox.test` as the underlying test (via the `test` argument) and all five alternative hypotheses: `"two.sided"`, `"less"`, `"greater"`, `"equivalence"`, and `"minimal.effect"`. The `mu` parameter works like `t.test` for standard alternatives (a single null value), but for equivalence and minimal effect testing a single value creates symmetric bounds (i.e., `mu = 0.5` becomes bounds at -0.5 and 0.5), or two values can be given for asymmetric bounds.

For instance, we can perform an equivalence test on the `sleep` data with symmetric bounds of $\pm$ 0.5:
...
Similarly, a minimal effect test on paired data from the `bugs` dataset can be performed:

```{r}
res_sh2 <- simple_htest(x = bugs$LDHF, y = bugs$LDLF,
                         paired = TRUE,
                         alternative = "minimal.effect", mu = 1)
res_sh2
```

The same interface works for non-parametric tests simply by changing the `test` argument:

```{r}
res_sh3 <- simple_htest(extra ~ group, data = sleep,
                         test = "wilcox.test",
                         alternative = "equivalence", mu = 0.5)
res_sh3
```

## Converting TOST Results with as_htest

Users who have already run `t_TOST` or `wilcox_TOST` and want to use the `htest` helper functions can convert their results with `as_htest`. This extracts the TOST-relevant test and packages it as a standard `htest` object.

```{r}
res1_htest <- as_htest(res1)
res1_htest
```

## Describing Results with describe_htest

For writing up results, `describe_htest` generates a formatted text string following typical scientific reporting conventions. It includes the significance decision, test statistic, p-value, estimate, confidence interval, and the hypothesis being tested. This works with any `htest` object — from base R or from TOSTER.

```{r}
describe_htest(simple_htest(extra ~ group, data = sleep))
```

This also works with standard R tests. Note the difference from the difference between the output below from `t.test` and `simple_htest` above. The differences, despite both descriptions coming from the same underlying statistical tests, are caused by differences in what output is produced by each function (i.e., `simple_htest` has slight differences in output from `t.test`).

```{r}
describe_htest(t.test(extra ~ group, data = sleep))
```

## Tabulating Results with df_htest

When combining results from multiple tests (e.g., running equivalence tests across several outcomes), it is useful to have results in a data frame. The `df_htest` function converts any `htest` object into a single-row data frame with standardized column names. Multiple results can then be combined with `rbind`.
...
## Plotting Results with plot_htest_est

A visual summary of the estimate and confidence interval can be produced with `plot_htest_est`. For equivalence and minimal effect tests, the equivalence bounds are shown as dashed vertical lines, making it easy to see whether the confidence interval falls within (or outside) the bounds.
...
For standard tests, only a single null reference line (at zero) appears:

```{r shplot2, fig.width=6, fig.height=3.5, fig.cap = "Estimate plot for a standard two-sided t-test."}
plot_htest_est(t.test(extra ~ group, data = sleep))
```
...
Therefore, it may be useful to explore alternatives to the t-test for TOST that either do not have those assumptions or are robust to violations of those assumptions.

The TOSTER package provides robust alternatives to the t-test organized into three categories.
First, there are *rank-based tests* — the Brunner-Munzel test (`brunner_munzel`) for stochastic equality and the Wilcoxon-Mann-Whitney TOST (`wilcox_TOST`) for location shift — which target entirely different estimands than means or medians.
Second, there are *resampling methods* — the bootstrap t-test (`boot_t_test`), permutation t-test (`perm_t_test`), and bootstrap TOST (`boot_t_TOST`) — which relax distributional assumptions while still testing hypotheses about means [@efron93].
Third, there are *log-transformed methods* — `log_TOST` and `boot_log_TOST` — which are parametric and bootstrap approaches for ratio-scale data commonly used in pharmaceutical bioequivalence studies [@he2022].
Where possible, the package now favors functions that return standard `htest` objects, which enforce a single hypothesis per test and integrate cleanly with the existing R ecosystem.

## Rank-Based Tests

Rank-based tests avoid distributional assumptions by operating on the ranks of the data rather than the raw values.
An important consideration is that these tests are *not* tests of means or medians — they target completely different estimands.
The choice between rank-based methods should therefore be driven by the estimand of interest: stochastic equality (i.e., the probability that a random observation from one group exceeds a random observation from another) for the Brunner-Munzel test, or pseudo-median of pairwise differences (under a location-shift assumption) for WMW tests.

### Brunner-Munzel Test

The Brunner-Munzel test [@brunner2000] provides a non-parametric test of stochastic equality.
Rather than testing whether two groups differ in location, it directly estimates the relative effect $\hat{p} = P(X > Y) + 0.5 \cdot P(X = Y)$, which is the probability that a randomly chosen observation from one group exceeds a randomly chosen observation from the other.
This estimand is interpretable without additional distributional assumptions, unlike the WMW test which requires a location-shift assumption to say anything about location differences.
@karch2021 argues that the Brunner-Munzel test should be preferred over the Mann-Whitney U test as the default nonparametric procedure because it is valid under heteroscedasticity and provides a more interpretable effect size.

The `brunner_munzel` function in TOSTER supports equivalence and minimal effect testing via the `alternative` argument, with bounds specified on the probability scale through the `mu` parameter.
Results can be reported on different scales via the `scale` argument ("probability", "difference", "logodds", or "odds"), and three test methods are available via `test_method`: "t" (default, t-approximation), "logit" (range-preserving CIs), and "perm" (studentized permutation, recommended for small samples; @neubert2007).
...
Within the TOST framework, there are two separate tests of directional location shift to determine if the location shift is within (equivalence) or outside (minimal effect) the equivalence bounds^[Care should be taken when considering paired samples; a test on the rank transformed data [@kornbrot1990rank] or another robust test may be more prudent.].

However, there are important caveats to the interpretation of WMW tests.
Many researchers mistakenly believe these are tests of medians, but this is not the case.
Without additional assumptions, the WMW test is a test of stochastic equality — identical to what the Brunner-Munzel test estimates.
Only under a location-shift assumption (the two distributions have identical shapes, differing only in location) does the WMW test become a test of the Hodges-Lehmann pseudo-median of pairwise differences, and only with additional symmetry can it be interpreted as a test of median or mean differences.
@median_test demonstrate several striking counterexamples: groups can have equal medians yet produce a significant WMW test (because distributions differ in shape), very different medians yet a non-significant WMW test (because neither group stochastically dominates the other), or even significance in the opposite direction from the median comparison.

This interpretive difficulty is especially pronounced for equivalence testing.
When performing TOST with Wilcoxon tests, the equivalence bounds are applied to the pseudo-median of pairwise differences (i.e., the Hodges-Lehmann estimator), which corresponds to the difference in means or medians only under the location shift assumption [@median_test].
When distributions differ in shape, bounds on the pseudo-median do not guarantee that means or medians are within those bounds.
For testing hypotheses about mean differences, `perm_t_test` or `boot_t_test` are more appropriate.
For stochastic superiority, `brunner_munzel` provides a clearer framework.
For testing effect sizes directly, `ses_calc` supports hypothesis testing on the standardized effect size such as the rank-biserial correlation.
The `wilcox_TOST` function remains useful for situations where rank-based tests are specifically desired, such as with ordinal data or when consistency with prior analyses is needed.

In the TOSTER package, `wilcox_TOST` operates similarly to `t_TOST`.
A standardized mean difference (SMD) is *not* calculated since this would be an inappropriate measure of effect size alongside the non-parametric test statistics.
...
The function can produce a rank-biserial correlation [@Kerby_2014], a WMW Odds [@wmwodds], or a "common language effect size" [@Kerby_2014] (also known as the non-parametric probability of superiority, or concordance probability)^[While `wilcox_TOST` does not have a dedicated plotting method, the result can be converted to an `htest` object with `as_htest`, or users can obtain an `htest` object directly via `simple_htest`. The `ses_calc` function also returns an `htest` object. Any of these `htest` objects can be plotted with `plot_htest_est`.].
...
Based on these results, we would conclude there is no significant difference but not equivalent differences either (i.e., inconclusive result).

\newpage

## Resampling Methods

When the goal is to test hypotheses about means or mean differences while relaxing distributional assumptions, resampling methods provide a powerful set of tools.
TOSTER provides two primary resampling approaches — bootstrap and permutation — each with distinct inferential logic.

The *bootstrap* [@efron93] resamples with replacement from the observed data to approximate the sampling distribution of the test statistic.
Bootstrap confidence intervals can be constructed using several methods (studentized, basic, percentile, or bias-corrected and accelerated), and the p-value computation is tied to the CI method via the CI inversion principle. With the way the package now works for bootstraps methods, a p-value is significant at level $\alpha$ if and only if the corresponding $1 - 2\alpha$ confidence interval excludes the null value.
This ensures consistency between reported CIs and p-values [@Thulin_2024].

The *permutation* approach shuffles group labels (for two-sample tests) or flips signs (for one-sample/paired tests) to construct the null distribution directly.
When the number of possible permutations is small enough, exact p-values can be computed; otherwise, a randomized approximation is used with the "+1" correction to avoid zero p-values [@phipson2010].
Studentized permutation tests [@janssen1997; @chung2013], which recompute the standard error for each permutation, provide valid inference even under heteroscedasticity *and* allow for accurate equivalence tests [@Arboretti_Pesarin_Salmaso_2020].

Both `boot_t_test` and `perm_t_test` return standard `htest` objects and support `alternative = "equivalence"` and `alternative = "minimal.effect"` for equivalence and minimal effect testing respectively.
Both also support Yuen's trimmed means via the `tr` parameter, providing additional robustness to outliers by trimming a specified fraction from each tail of the distribution before computing means and variances.

### Bootstrap t-test

The `boot_t_test` function provides a bootstrap t-test that returns a standard `htest` object.
The `boot_ci` argument controls the CI method ("stud", "basic", "perc", or "bca"), and as noted above, the p-value method follows the CI method to maintain consistency.
Equivalence bounds are specified via the `mu` argument (two values for equivalence/minimal effect testing).
...
The `perm_t_test` function provides a permutation-based t-test.
When `R = NULL`, the function attempts to enumerate all possible permutations for an exact test; otherwise, it draws `R` random permutations.
By default, the test is studentized (`perm_se = TRUE`), meaning the standard error is recomputed for each permuted sample, which provides valid inference under heteroscedasticity [@janssen1997; @chung2013].
For randomized permutations, the "+1" correction is applied by default to ensure proper type I error control [@phipson2010].
...
For situations where both a two-tailed nil hypothesis test and an equivalence test are desired simultaneously, the `boot_t_TOST` function provides both in a single call, returning a `TOSTt` object.
This function uses the same bootstrap methods as `boot_t_test` and additionally provides bootstrapped SMD estimates [@Kirby2013].
...
An alternative to using the dedicated `log_TOST` function is to simply log-transform the data and analyze it with `t_TOST` (or any of the resampling methods described above). The differences on the log scale can then be interpreted as "sympercents" — symmetric percentage differences on the $100 \cdot \ln$ scale [@cole2000]. Sympercents have the attractive property that a difference of +10 sympercent and -10 sympercent are symmetric and of equal magnitude, unlike ordinary percentages where +10% and -10% are not. This approach provides more flexibility (e.g., one-sample tests become possible) at the cost of requiring the user to specify equivalence bounds on the log scale rather than as a ratio.
...
The bootstrap version of `log_TOST`, `boot_log_TOST`, uses the same bootstrap methods as `boot_t_test`, but on the log-transformed values, producing the ratio of means as the effect size.
...
currently not available, and users would instead have to run 2 one-sided tests
...
In this manuscript I have demonstrated most of the newest functions and features within the TOSTER R package. 
...
due to the chosen analysis. If your interpretation of the results substantially changes depending on the test, then it would be prudent to explore what features in the data, and the underlying data generating process, might explain this discrepancy.
...
All analyses/code in this manuscript are from TOSTER v0.9.0:

```
# Install the exact release with this code
devtools::install_github("Lakens/TOSTER@v0.9.0")
```
