...
trueP2_num= NA, 
power=NA, 
as.matrix(expand.grid(Nvar=c(2,4),sigma2=c(0.4,0.5,1), Nsample=c(120, 1200,12000))))
...
X1 <- rep(basematrix$X1,2000000)
X2 <- rep(basematrix$X2,2000000)
X3 <- rep(basematrix$X3,2000000)
X4 <- rep(basematrix$X4,2000000)
...
library(tidyr)
problist_long<-gather(problist_df,JJJindex,pr_less_alpha,jjj1:jjj18, factor_key=TRUE)


resultsmatall<-merge(resultsmat,problist_long, by="JJJindex",all=TRUE)

resultsmatall$P2 <-as.factor(round(ceiling_dec(resultsmatall$trueP2_v2,3),3))
...
library(ggplot2)
qplot(x=Delta, y= pr_less_alpha, group= g, pch=N, lty=N, col= P2, data= resultsmatall)+geom_line()+  geom_hline(yintercept = 0.05)+ facet_grid(Nvar ~ . ) + scale_x_continuous(breaks = seq(0, 0.1, by = 0.01))  + scale_y_continuous(breaks = seq(0, 1, by = 0.05))+  coord_cartesian(ylim = c(0, 0.20))+ labs(x = expression(Delta), y = expression("probability of p" < alpha))

####  plot with truncated axis:
library(ggplot2)
qplot(x=Delta, y= pr_less_alpha, group= g, pch=N, lty=N, col= P2, data= resultsmatall)+geom_line()+  geom_hline(yintercept = 0.05)+ facet_grid(Nvar ~. ) + scale_x_continuous(breaks = seq(0, 0.1, by = 0.01))  + scale_y_continuous(breaks = seq(0, 1, by = 0.1))+ labs(x = expression(Delta),y = expression("probability of p" < alpha))+ coord_cartesian(ylim = c(0, 1))
