...
1. **Sample sizes are reported** directly in the output (base R does not show *n*).
...
Notice that `simple_htest` reports the sample sizes in each group and appends the mean difference to the output, saving the reader a calculation step.
...
The output includes the Hodges-Lehmann estimate of the location shift and, again, sample sizes.
...
`z_cor_test()` tests correlations using Fisher's z-transformation and supports equivalence bounds on the correlation coefficient:
