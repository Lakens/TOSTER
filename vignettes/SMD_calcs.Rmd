...
We would  recommend using "nct" as the default for any analysis but as @viechtbauer2007approximate points out the approximate methods usually suffice.
...
It is my belief that SMDs provide another interesting quantitative effect size description, 
but have dubious and limited inferential utility^[see @Senn_2011 and @Baguley_2009; though many may disagree]. 
...
bootstrapping approach [see `boot_smd_calc` @Kirby2013] as it is a more robust option.
...
Hedges g (bias corrected SMD) can then be calculated by
multiplying the SMD by J
...
recommend using bootstrapping techniques rather than any of these approximations whenever possible [@Kirby2013], and these method are available in the `boot_smd_calc` function.
...
@Algina_Keselman_Penfield_2005 proposed a robust alternative that uses **trimmed means** for location and **Winsorized variances** for scale, with a rescaling constant to maintain comparability with Cohen's $\delta$ under normality.
...
$$
w_i = \begin{cases}
x_{(g+1)} & \text{if } x_i \leq x_{(g+1)} \\
x_i & \text{if } x_{(g+1)} < x_i < x_{(n-g)} \\
x_{(n-g)} & \text{if } x_i \geq x_{(n-g)}
\end{cases}
$$
...
**Rescaling constant.** To ensure the robust SMD equals Cohen's $\delta$ under normality, a rescaling constant $c(\gamma)$ is applied [@Algina_Keselman_Penfield_2005]. Let $a = \Phi^{-1}(1 - \gamma)$ where $\Phi^{-1}$ is the standard normal quantile function and $\phi$ is the standard normal density. Then:
