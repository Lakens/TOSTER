...
For more robust analyses when raw data is available, TOSTER provides the `boot_cor_test` function. This bootstrapping approach generally produces more reliable results than Fisher's z-based tests, especially with non-normal data or when outliers are present.
...
1. **If raw data is available:**
   ...
   - When outliers or non-normality are concerns, consider the robust methods (winsorized or percentage bend)
