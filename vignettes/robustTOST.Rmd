...
The `hodges_lehmann` function provides a robust location test based on the Hodges-Lehmann estimator [@hodges1963]. This is a natural companion to the WMW discussion above: while `wilcox_TOST` implicitly relies on the location-shift assumption without making it explicit, `hodges_lehmann` directly estimates and tests the location shift parameter with transparent assumptions.

## Why Use the Hodges-Lehmann Estimator?

The Hodges-Lehmann estimator has several appealing properties:

1. **High breakdown point (~29.3%):** The estimate remains stable even when nearly 30% of observations are outliers, making it far more robust than the mean.
...
The `se_method` argument controls how the standard error is computed:

- **`"agresti"`** (default): Uses the Agresti/Lehmann placement-based variance estimate. Hypothesis tests are conducted on the log-odds scale, which has better asymptotic properties (unbounded, faster convergence to normality). The reported effect size and CI are back-transformed to the requested scale.

- **`"fisher"`**: Uses the Fisher z-transformation (legacy method). This is the same approach used in earlier versions of the package.
...
| **Hodges-Lehmann** | Robust location (~29% breakdown), explicit estimand | Requires location-shift assumption for full interpretation | Robust location shift testing |
