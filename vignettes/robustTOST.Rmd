...
| Brunner-Munzel | `brunner_munzel()` with `simple_htest()` | Probability-based, robust to heteroscedasticity | Stochastic superiority is the effect of interest |
...
When `trim > 0`, the function uses trimmed means and winsorized variances. Let $g = \lfloor \gamma \cdot n \rfloor$ be the number of observations trimmed from each tail (where $\gamma$ is the trimming proportion), and let $h = n - 2g$ be the effective sample size.
...
|--------|---------------|-------------|---------------|
| **Wilcoxon TOST** | Simple, widely accepted | Ambiguous hypothesis; not clearly about means | Ordinal data; legacy analyses |
