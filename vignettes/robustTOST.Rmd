...
In this package there are currently 2 functions that provide robust alternatives to the `t_TOST` function. The 
...
The confidence intervals can then be calculated as the following:

$$

r_z = atanh(r_{biseriall})
\\
...
$$
The same process is completed for the one sample case but with the one sample solution for the equation outlined by $t(z^{*b})$. The paired sample case in this bootstrap procedure is equivalent to the one sample soluation because the test is based on the difference scores.
