...
A common misconception is that the WMW test compares medians [see @karch2021; @divine2018]. **Without additional assumptions**, the WMW test is a test of stochastic equality:
...
**For differences in means (or trimmed means):** Use `perm_t_test` or `boot_t_TOST`. These test hypotheses directly about means while relaxing distributional assumptions.
...
The Brunner-Munzel test can be used for non-inferiority testing in clinical trials with ordered categorical data [@munzel2003]. By setting appropriate bounds, you can test whether a new treatment is not relevantly inferior to a standard:
